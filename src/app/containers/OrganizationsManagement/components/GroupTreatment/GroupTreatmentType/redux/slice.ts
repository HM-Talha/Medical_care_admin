...
import { GroupTreatmentItemType, UserState } from "../types";
import set from "lodash/set";
import { FormError } from "app/containers/types";

const initialState = UserState;

export const GroupTreatmentSlice = createSlice({
  name: "groupTreatmentState",
  initialState,
  reducers: {
    editUser: (state, action: PayloadAction<GroupTreatmentItemType>) => {
    ...
    updateSuccess(state, action: PayloadAction<GroupTreatmentItemType>) {
      state.loading = false;
    ...
    handleBlockSuccess: (state, action: PayloadAction<GroupTreatmentItemType>) => {
      // const index = state.list.items.findIndex(
...
export const { reducer, actions, name: sliceKey } = GroupTreatmentSlice;
