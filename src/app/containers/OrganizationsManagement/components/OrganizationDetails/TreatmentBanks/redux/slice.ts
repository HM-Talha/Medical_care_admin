...
import { TreatmentBanksListItemType, UserState } from "../types";
import set from "lodash/set";
import { FormError } from "app/containers/types";

const initialState = UserState;

export const TreatmentBankListSlice = createSlice({
  name: "treatmentBanksListState",
  initialState,
  reducers: {
    editUser: (state, action: PayloadAction<TreatmentBanksListItemType>) => {
    ...
    updateSuccess(state, action: PayloadAction<TreatmentBanksListItemType>) {
      state.loading = false;
    ...
    handleBlockSuccess: (state, action: PayloadAction<TreatmentBanksListItemType>) => {
      // const index = state.list.items.findIndex(
...
export const { reducer, actions, name: sliceKey } = TreatmentBankListSlice
;
