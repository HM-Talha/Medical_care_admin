...
import { PatientsListItemType, UserState } from "../types";
...
export const PatientListSlice = createSlice({
  ...
  initialState,
  reducers: {
    editUser: (state, action: PayloadAction<PatientsListItemType>) => {
    ...
    updateSuccess(state, action: PayloadAction<PatientsListItemType>) {
      state.loading = false;
    ...
    handleBlockSuccess: (state, action: PayloadAction<PatientsListItemType>) => {
      // const index = state.list.items.findIndex(
