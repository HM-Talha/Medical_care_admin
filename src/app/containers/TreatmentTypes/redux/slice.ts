...
import { UserState, TreatmentTypeItemType } from "../types";
...
export const treatmentTypeSlice = createSlice({
  ...
  initialState,
  reducers: {
    editUser: (state, action: PayloadAction<TreatmentTypeItemType>) => {
    ...
    updateSuccess(state, action: PayloadAction<TreatmentTypeItemType>) {
      state.loading = false;
    ...
    handleBlockSuccess: (state, action: PayloadAction<TreatmentTypeItemType>) => {
      // const index = state.list.items.findIndex(
