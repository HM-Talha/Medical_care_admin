...
import { PushNotificationsListItemType, NotificationState } from "../types";
...
export const pushNotificationSlice = createSlice({
  ...
  initialState,
  reducers: {
    editUser: (state, action: PayloadAction<PushNotificationsListItemType>) => {
    ...
    updateSuccess(state, action: PayloadAction<PushNotificationsListItemType>) {
      state.loading = false;
    ...
    handleBlockSuccess: (state, action: PayloadAction<PushNotificationsListItemType>) => {
      // const index = state.list.items.findIndex(
